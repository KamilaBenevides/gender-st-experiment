#' ---
#' title: "ANCOVA test for `prevention.pos`~`prevention.pre`+`Condition`"
...
#' 
## ---- echo=FALSE--------------------------------------------------------------
car::Boxplot(`prevention.pos` ~ `Condition`, data = dat[["prevention.pos"]], id = list(n = Inf))
...
#' ### Textual Report
#' 
#' After controlling the linearity of covariance "prevention.pre", ANCOVA tests with independent between-subjects variables "Condition" (inThreat, inBoost, control) were performed to determine statistically significant difference on the dependent varibles "prevention.pos". For the dependent variable "prevention.pos", there was not statistically significant effects.
#' 
#' 
#' 
#' 
